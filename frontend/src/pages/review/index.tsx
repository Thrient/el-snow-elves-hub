...
import { Button, Card, Spin, Empty, Modal, Input, Pagination, Tag, message, Tabs } from "antd";
import {
  EyeOutlined,
  CheckOutlined,
  CloseOutlined,
...
const CONTENT_TYPE_META: Record<string, { label: string; color: string; icon: FC<{ className?: string }> }> = {
  post: { label: "帖子", color: "#1677ff", icon: FileTextOutlined },
  reply: { label: "回复", color: "#52c41a", icon: MessageOutlined },
  task: { label: "任务", color: "#faad14", icon: ToolOutlined },
  comment: { label: "评论", color: "#13c2c2", icon: CommentOutlined },
};
...
const ReviewPage: FC = () => {
  ...
  const load = useCallback((p = page, s = status) => {
    setLoading(true);
    reviewApi
      .list({ page: p, size: pageSize, status: s })
      .then((res) => {
        setItems(res.items);
        setTotal(res.total);
      })
